...
The scripts support custom environments as well; however, there are couple of points to note before you start constructing your environemnt:

1. In most of the code, we have assumed that the user is using a gymnasium (introduced by OpenAI) environemnt to train on. So if you want to make your own environemnt, it is best to write it as a custom gymnasium environemnt.

2. The environemnt should have a `reset(seed)` command in which by giving a certain seed, it resets the environemnt to the initial conditions generated by that seed only.

3. It should have a `step(action)` function that takes a single step in the environemnt using this action. This function should return following variables respectively:

    - environment state (observation): The state of the environment after the step is taken, should be an array.

    - reward: The reward that the agent acquired after taking the step, should be float or int.

    - terminated: If the episode is terminated, returns true, else it returns false.
